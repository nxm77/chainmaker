...
import (
	...
	"chainmaker_web/src/sync/datacache"
...
func (getLatestTxListHandler *GetLatestTxListHandler) Handle(ctx *gin.Context) {
	...
	//从缓存获取最新的交易数据
	txList, err := datacache.GetLatestTxListCache(params.ChainId)
	if err != nil {
...
func (GetContractTxListHandler *GetContractTxListHandler) Handle(ctx *gin.Context) {
	...
	if params.ContractAddr != "" {
		contractInfo, err = dbhandle.GetContractByAddr(chainId, params.ContractAddr)
	} else {
		contractInfo, err = dbhandle.GetContractByName(chainId, params.ContractName)
	}
	if err != nil {
		ConvergeHandleFailureResponse(ctx, err)
		return
	...
	if len(userAddrs) > 0 || params.ContractMethod != "" || *params.TxStatus >= 0 {
		txCount, err = dbhandle.GetContractTxCount(chainId, contractInfo.NameBak, params.ContractMethod,
	...
	//获取交易列表
	contractTxIDList, err := dbhandle.GetContractTxIDList(params.Offset, params.Limit, chainId, contractInfo.NameBak,
...
func (GetBlockTxListHandler *GetBlockTxListHandler) Handle(ctx *gin.Context) {
	...
	//获取交易列表
	blockTxInfoList, err := dbhandle.GetBlockTxListByHash(params.ChainId, params.BlockHash, params.Offset, params.Limit)
	if err != nil {
		ConvergeHandleFailureResponse(ctx, err)
		return
	}

	if len(blockTxInfoList) == 0 {
		ConvergeListResponse(ctx, []interface{}{}, 0, nil)
		return
	}
	...
	//获取账户BNS地址
	var accountAddrs []string
	for _, txInfo := range blockTxInfoList {
		accountAddrs = append(accountAddrs, txInfo.UserAddr)
	}
	accountMap, _ := dbhandle.QueryAccountExists(params.ChainId, accountAddrs)
...
func getTxTotalCount(params *entity.GetTxListParams, userAddrs, senders []string) (int64, error) {
	...
	if params.StartTime == 0 && params.TxId == "" && len(userAddrs) == 0 &&
		len(senders) == 0 && *params.TxStatus == -1 {
		totalCount, err = dbhandle.GetTotalTxNumByAccount(params.ChainId)
		if err != nil {
			log.Errorf("GetTxList GetTotalTxNumByAccount err:%v", err)
		}
...
func SetTxNumByTimeCache(ctx *gin.Context, chainId, startKey, endKey string, interval int64, txMap map[int64]int64) {
	...
	// 设置键值对和过期时间
	_ = cache.GlobalRedisDb.Set(ctx, redisKey, string(retJson), 10*time.Minute).Err()
}
...
// GetQueryTxListHandler get
type GetQueryTxListHandler struct {
}

// Handle deal
func (handler *GetQueryTxListHandler) Handle(ctx *gin.Context) {
	params := entity.BindGetQueryTxListHandler(ctx)
	if params == nil || !params.IsLegal() {
		newError := entity.NewError(entity.ErrorParamWrong, "param is wrong")
		ConvergeFailureResponse(ctx, newError)
		return
	...
	//获取交易id列表
	txCount, txIdList, err := dbhandle.GetQueryTxIDList(params)
	if err != nil {
		log.Errorf("GetTxList err : %s", err.Error())
		ConvergeHandleFailureResponse(ctx, err)
		return
	...
	contractTxList, err := dbhandle.GetContractTransactionList(params.ChainId, txIdList)
	if err != nil {
		log.Errorf("GetContractTransactionList err : %s", err.Error())
		ConvergeHandleFailureResponse(ctx, err)
		return
	...
	txsViewInterface := BuildContractTxView(params.ChainId, contractTxList)
	ConvergeListResponse(ctx, txsViewInterface, txCount, nil)
}
