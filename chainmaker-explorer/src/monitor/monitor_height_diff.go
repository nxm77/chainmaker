...
func monitorHeightDiff(sdkClients []*client.SdkClient) {
...
// 区块高度监控
func singleHandle(sdkClient *client.SdkClient) string {
	...
	//nolint:gosec
	bcHeight := int64(block.Block.Header.BlockHeight)
	...
	if heightDiff > heightDiffConf {
		...
		msg := GetBackwardDiffMsg(chainId, nodeRemotes, heightDiff, bcHeight, dbHeight, timestamp)
		explorerAfterChain.WithLabelValues(sdkClient.ChainId,
			strconv.FormatInt(dbHeight, 10), strconv.FormatInt(bcHeight, 10)).Inc()
