...
func GetMonitorSdkClients(monitorChains []*config.ChainInfo) [][]*client.SdkClient {
	clients := make([][]*client.SdkClient, 0)
	for _, chainInfo := range monitorChains {
		sdkClients := make([]*client.SdkClient, 0)
		tempChain := chainInfo
		for _, node := range chainInfo.NodesList {
			...
			chainClient, err := client.CreateChainClient(tempChain)
			sdkClient := client.NewSdkClient(tempChain, chainClient)
			if err != nil {
...
// Monitor 监控链指定节点
func Monitor(sdkClients []*client.SdkClient, index int) {
	//初始化配置
	//节点高度差检测
...
func MonitorChain(allSdkClients [][]*client.SdkClient) {
	//敏感词监控
	go monitorSafeWord()
	//单用户短时间大批量交易探测
	go monitorTx()
