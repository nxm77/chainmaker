...
func BatchInsertNode(chainId string, nodeList []*db.Node) error {
	...
	//更新节点
	deErr := UpdateNode(chainId, updateNodes)
	if deErr != nil {
...
func GetNodeInById(chainId string, nodeIds []string) (map[string]*db.Node, error) {
	...
	conditions := []QueryCondition{
		{Field: "nodeId", Value: nodeIds, Condition: "in"},
	}
...
// @param ${param}
// @return error
func UpdateNode(chainId string, nodes []*db.Node) error {
	nodeJson, _ := json.Marshal(nodes)
	log.Infof("update node status, chainId : %s, nodes : %s", chainId, string(nodeJson))
	...
	for _, nodeInfo := range nodes {
		...
		if err != nil {
			log.Errorf("update node status fail, chainId : %s, nodeId : %s , cause: %s",
				chainId, nodeInfo.NodeId, err.Error())
			return err
		}
	}
	return nil
...
func GetNodeList(chainId, orgId, nodeId string, offset, limit int) ([]*db.Node, int64, error) {
	nodes := make([]*db.Node, 0)
	var count int64
	where := map[string]interface{}{
		"status": config.StatusNormal,
