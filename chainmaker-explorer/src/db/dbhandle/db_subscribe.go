...
func GetSubscribeByChainIds(chainIds []string) ([]*db.Subscribe, error) {
	...
	conditions := []QueryCondition{
		{Field: "chainId", Value: chainIds, Condition: "in"},
	}
	query := BuildQuery(db.TableSubscribe, conditions)
	err := query.Find(&subscribe).Error
	if err != nil {
...
func UpdateSubscribe(subscribe *db.Subscribe) error {
	...
	params := map[string]interface{}{
		...
		"userSignKey": subscribe.UserSignKey,
		"userSignCrt": subscribe.UserSignCrt,
		"userTlsKey":  subscribe.UserTlsKey,
		"userTlsCrt":  subscribe.UserTlsCrt,
		"userEncKey":  subscribe.UserEncKey,
		"userEncCrt":  subscribe.UserEncCrt,
...
func BuildSubscribeInfo(chainInfo *config.ChainInfo, status int) *db.Subscribe {
	...
	if chainInfo.UserInfo != nil {
		subscribeInfo.UserSignKey = chainInfo.UserInfo.UserSignKey
		subscribeInfo.UserSignCrt = chainInfo.UserInfo.UserSignCrt
		subscribeInfo.UserTlsKey = chainInfo.UserInfo.UserTlsKey
		subscribeInfo.UserTlsCrt = chainInfo.UserInfo.UserTlsCrt
		subscribeInfo.UserEncKey = chainInfo.UserInfo.UserEncKey
		subscribeInfo.UserEncCrt = chainInfo.UserInfo.UserEncCrt
	}
...
func InsertOrUpdateSubscribe(chainInfo *config.ChainInfo, status int) error {
	...
	subscribeInfo := BuildSubscribeInfo(chainInfo, status)
	if subscribe == nil {
