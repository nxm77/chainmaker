...
func UpdateCrossSubChainById(chainId string, subChainInfo *db.CrossSubChainData) error {
	...
	if subChainInfo.Timestamp > 0 {
		params["timestamp"] = subChainInfo.Timestamp
	}
	err := db.GormDB.Table(tableName).Where(where).Updates(params).Error
	if err != nil {
		return err
...
func GetCrossSubChainById(chainId string, subChainIds []string) (map[string]*db.CrossSubChainData, error) {
	...
	conditions := []QueryCondition{
		{Field: "subChainId", Value: subChainIds, Condition: "in"},
	}
	query := BuildQuery(tableName, conditions)
	err := query.Find(&crossSubChains).Error
	if err != nil {
...
func GetCrossSubChainList(offset, limit int, chainId, subChainId string) (
