...
func TestGetFungibleContractList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := GetFungibleContractList(tt.args.offset, tt.args.limit, tt.args.chainId, tt.args.contractKey, "desc", "txNum")
			if (err != nil) != tt.wantErr {
		...
		},
		{
			...
			args: args{
				...
				contract: &db.FungibleContract{
					...
					BlockHeight:  123456,
					TransferNum:  1,
					TotalSupply:  GetAmountDecimal("1234"),
				},
			},
			wantErr: false,
		},
...
func TestGetFTContractByNameOrAddr(t *testing.T) {
	// Test case 1: Getting contract byte code by transaction id successfully
	_, err1 := GetFTContractByNameOrAddr(db.UTchainID, "key")
	if err1 != nil {
