...
func GetUserListByAdder(chainId string, adders []string) (map[string]*db.User, error) {
	...
	conditions := []QueryCondition{
		{Field: "userAddr", Value: adders, Condition: "in"},
	}
...
func GetUserList(offset, limit int, chainId, orgId string, userIds, userAddrs []string) ([]*db.User, int64, error) {
	...
	conditions := []QueryCondition{}
	if orgId != "" {
		conditions = append(conditions, QueryCondition{Field: "orgId", Value: orgId, Condition: "="})
	}
	if len(userIds) > 1 {
		conditions = append(conditions, QueryCondition{Field: "userId", Value: userIds, Condition: "IN"})
	} else if len(userIds) == 1 && userIds[0] != "" {
		conditions = append(conditions, QueryCondition{Field: "userId", Value: "%" + userIds[0] + "%", Condition: "LIKE"})
	}
	if len(userAddrs) > 1 {
		conditions = append(conditions, QueryCondition{Field: "userAddr", Value: userAddrs, Condition: "IN"})
	} else if len(userAddrs) == 1 && userAddrs[0] != "" {
		conditions = append(conditions, QueryCondition{Field: "userAddr", Value: "%" + userAddrs[0] + "%", Condition: "LIKE"})
	}
