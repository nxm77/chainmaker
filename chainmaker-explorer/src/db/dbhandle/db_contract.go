...
func UpdateContract(chainId string, contract *db.Contract) error {
	...
	//删除合约缓存
	//DelContractInfoCache(chainId, contract.Addr, contract.NameBak)
	return nil
}
...
func UpdateContractTxNum(chainId string, contract *db.Contract) error {
	...
	//更新合约缓存
	//UpdateContractNumCache(chainId, contract)
	return nil
}
...
func GetContractNum(chainId string) (int64, error) {
	...
	tableName := db.GetTableName(chainId, db.TableContract)
	conditions := []QueryCondition{
		{Field: "contractStatus", Value: SystemContractStatus, Condition: ">"},
	}
...
func GetContractList(chainId string, offset, limit int, status *int32, runtimeType, contractKey,
	contractType, order, orderBy string, creators, creatorAddrs, upgraders, upgradeAddrs []string) (
	[]*db.Contract, int64, error) {
	var count int64
	contracts := make([]*db.Contract, 0)
	...
	if runtimeType != "" {
		conditions = append(conditions, QueryCondition{Field: "runtimeType", Value: runtimeType, Condition: "="})
	}
	if contractType != "" {
		conditions = append(conditions, QueryCondition{Field: "contractType", Value: contractType, Condition: "="})
	}
	if status != nil && *status != -1 {
		conditions = append(conditions, QueryCondition{Field: "contractStatus", Value: *status, Condition: "="})
	} else {
		conditions = append(conditions, QueryCondition{Field: "contractStatus", Value: SystemContractStatus, Condition: ">"})
	}
	if len(creators) > 1 {
		conditions = append(conditions, QueryCondition{Field: "createSender", Value: creators, Condition: "IN"})
	} else if len(creators) == 1 {
		valueStr := "%" + creators[0] + "%"
		conditions = append(conditions, QueryCondition{Field: "createSender", Value: valueStr, Condition: "LIKE"})
	}

	if len(creatorAddrs) > 0 {
		conditions = append(conditions, QueryCondition{Field: "creatorAddr", Value: creatorAddrs, Condition: "IN"})
	}
	if len(upgraders) > 0 {
		conditions = append(conditions, QueryCondition{Field: "upgrader", Value: upgraders, Condition: "IN"})
	}
	if len(upgradeAddrs) > 0 {
		conditions = append(conditions, QueryCondition{Field: "upgradeAddr", Value: upgradeAddrs, Condition: "IN"})
	}
	if contractKey != "" {
		valueStr := "%" + contractKey + "%"
		conditions = append(conditions, QueryCondition{Field: "contractKey", Value: valueStr, Condition: "LIKE"})
	}
...
func GetChainMainDIDContract(chainId string) *db.Contract {
	contractTypeDID := standard.ContractStandardNameCMDID
	//从数据库获取
	var contract *db.Contract
	// 构建查询条件
	tableName := db.GetTableName(chainId, db.TableContract)
	conditions := []QueryCondition{
		{Field: "contractType", Value: contractTypeDID, Condition: "="},
		{Field: "timestamp", Condition: "asc"},
	}
	query := BuildQuery(tableName, conditions)
	err := query.First(&contract).Error
	if err != nil {
		log.Warnf("GetChainMainDIDContract err, cause : %v", err)
	}
...
// GetContractByName 根据合约名称获取合约
func GetContractTypes(chainId string) ([]string, error) {
	if chainId == "" {
		return nil, db.ErrTableParams
	}
	...
	tableName := db.GetTableName(chainId, db.TableContract)
	err := db.GormDB.Table(tableName).Distinct("contractType").Pluck("contractType", &contractTypes).Error
	if err != nil {
		return nil, err
	}
