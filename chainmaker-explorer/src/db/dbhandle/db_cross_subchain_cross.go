	...
	for _, v := range insertList {
		...
		if len(crossCache) > 0 {
			...
			for _, cross := range crossCache {
				if cross.CrossChainId == v.CrossChainId {
					exists = true
					break
				}
			}
...
func UpdateCrossSubChainCross(chainId string, subChainCross *db.CrossSubChainCrossChain) error {
	...
	where := map[string]interface{}{
		...
		"crossChainId": subChainCross.CrossChainId,
	}
	...
	for _, v := range cachedSubChainCrossList {
		if v.CrossChainId == subChainCross.CrossChainId {
...
func GetCrossSubChainCrossNum(chainId string, subChainIds []string) ([]*db.CrossSubChainCrossChain, error) {
	...
	conditions := []QueryCondition{
		{Field: "subChainId", Value: missingChainIds, Condition: "in"},
	}
	query := BuildQuery(tableName, conditions)
	err := query.Find(&findCrossSubChains).Error
	if err != nil {
