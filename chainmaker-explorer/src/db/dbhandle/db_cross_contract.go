...
func InsertCrossContract(chainId string, insertList []*db.CrossChainContract) error {
	if chainId == "" || len(insertList) == 0 {
		return nil
	}
	...
	for _, contract := range insertList {
		//删除缓存
		ctx := context.Background()
		prefix := config.GlobalConfig.RedisDB.Prefix
		redisKey := fmt.Sprintf(cache.RedisCrossContractCount, prefix, chainId, contract.SubChainId)
...
func SetCrossContractCountCache(chainId, subChainId string, contractCount int64) {
	...
	redisKey := fmt.Sprintf(cache.RedisCrossContractCount, prefix, chainId, subChainId)
	// 设置键值对和过期时间(24h 过期)
	_ = cache.GlobalRedisDb.Set(ctx, redisKey, contractCount, 10*time.Minute).Err()
}
