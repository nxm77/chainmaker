...
func TestGetFungiblePositionByOwners(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := GetFungiblePositionByOwners(tt.args.chainId, tt.args.ownerAddr)
...
func TestGetNFTPositionJoinAccount(t *testing.T) {
	_, err := GetNFTPositionJoinAccount(0, 2, db.UTchainID, "contractAddr", "ownerAddr")
	if err != nil {
		t.Errorf("Test case 1 failed: %v", err)
	}
}

func TestGetNFTPositionByAddrJoinAccount(t *testing.T) {
	_, err := GetNFTPositionByAddrJoinAccount(db.UTchainID, "contractAddr", []string{"ownerAddr"})
	if err != nil {
		t.Errorf("Test case 1 failed: %v", err)
	}
}
