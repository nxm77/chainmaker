...
func GetBlockList(offset int, limit int, chainId string, blockKey, nodeId string) ([]*db.Block, error) {
	blockList := make([]*db.Block, 0)
	if chainId == "" {
...
func GetBlockListCount(chainId string, blockKey, nodeId string) (int64, error) {
	if chainId == "" {
	...
	var totalCount int64
	tableName := db.GetTableName(chainId, db.TableBlock)
	err := db.GormDB.Table(tableName).Where(where).Count(&totalCount).Error
	if err != nil {
		return 0, err
	}
	return totalCount, nil
...
func GetBlockByStatus(chainId string, status int) ([]*db.Block, error) {
	...
	conditions := []QueryCondition{
		{Field: "delayUpdateStatus", Value: status, Condition: "="},
	}
