...
func TestGetNonFungibleContractList(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, _, err := GetNonFungibleContractList(tt.args.offset, tt.args.limit, tt.args.chainId, tt.args.contractKey, "desc", "txNum")
			if (err != nil) != tt.wantErr {
		...
		},
		{
			...
			args: args{
				...
				contract: &db.NonFungibleContract{
					...
					TotalSupply:     GetAmountDecimal("12"),
					HolderCount:     2,
					TransferNum:     1,
					BlockHeight:     123456,
				},
...
func TestGetNFTContractByNameOrAddr(t *testing.T) {
	// Test case 1: Getting contract byte code by transaction id successfully
	_, err1 := GetNFTContractByNameOrAddr(db.UTchainID, "key")
	if err1 != nil {
