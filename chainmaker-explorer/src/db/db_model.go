...
const (
	//DIDStatusSuccess 状态成功
	DIDStatusSuccess = 0
	//DIDStatusBlack 状态黑名单
	DIDStatusBlack = 1
...
type GetContractResult struct {
	...
	InsertEventTopic  []*ContractEventTopic
	UpdateEventTopic  []*ContractEventTopic
}
...
type DIDAccountDate struct {
	ID      string `json:"id"`
	Address string `json:"address"`
}

type DIDSaveData struct {
	SaveDIDDetails   []*DIDDetail
	InsertDIDHistory []*DIDSetHistory
	UpdateDIDStatus  map[string]int
	UpdateDIDIssuer  map[string]bool
	SaveVCTemplates  []*VCTemplate
	InsertVCHistory  []*VCIssueHistory
	UpdateVCStatus   map[string]int
}

func NewDIDSaveData() *DIDSaveData {
	return &DIDSaveData{
		SaveDIDDetails:   make([]*DIDDetail, 0),
		InsertDIDHistory: make([]*DIDSetHistory, 0),
		UpdateDIDStatus:  make(map[string]int),
		UpdateDIDIssuer:  make(map[string]bool),
		SaveVCTemplates:  make([]*VCTemplate, 0),
		InsertVCHistory:  make([]*VCIssueHistory, 0),
		UpdateVCStatus:   make(map[string]int),
	}
}

type CrossContractByTransfer struct {
