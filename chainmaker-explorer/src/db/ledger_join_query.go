...
// BlockTxListResult 跨链交易连表查询
type BlockTxListResult struct {
	TxId         string `json:"txId" gorm:"column:txId"`
	TxIndex      int    `json:"txIndex" gorm:"column:txIndex"`
...
type InvocationInfo struct {
	InvokingContract string `gorm:"column:invokingContract"`
	InvokingMethod   string `gorm:"column:invokingMethod"`
}

type MethodTargets struct {
	InvokingMethod  string `gorm:"column:invokingMethod"`
	TargetContracts string `gorm:"column:targetContracts"`
}

// CycleJoinTransferResult 跨链交易连表查询
//
//nolint:govet
type CycleDurationList struct {
