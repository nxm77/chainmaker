...
var (
	...
	TableContractEventTopic         = `contract_event_topic`
	TableUser                       = "user"
	TableOrg                        = "org"
	TableNode                       = "node"
	TableContractUpgradeTransaction = "contract_upgrade_transaction"
	TableContractByteCode           = "contract_byte_code"
	TableGas                        = "gas"
	...
	TableContractVerifyResult = "contract_verify_result"
	TableContractSourceFile   = "contract_source_file"

	TableDIDDetail      = "did_detail"
	TableDIDSetHistory  = "did_set_history"
	TableVCIssueHistory = "vc_issue_history"
	TableVCTemplate     = "vc_template"

	TableLoginUser      = "login_user"
	TableLoginUserToken = "login_user_token"

	TableContractABIFile              = "contract_abi_file"
	TableContractABITopic             = "contract_abi_topic"
	TableContractCrossCall            = "contract_cross_call"
	TableContractCrossCallTransaction = "contract_cross_call_transaction"
)
...
func GetTableName(chainId, table string) string {
	prefix := config.GlobalConfig.DBConf.Prefix
	table = prefix + table
...
func GetBlockTableNames() []TableInfo {
	return []TableInfo{
		...
		{
			Name:        TableContractEventTopic,
			Structure:   &ContractEventTopic{},
			Description: "合约交易事件topic表",
		},
		{
			Name:        TableUser,
			Structure:   &User{},
		...
		{
			Name:        TableContractByteCode,
			Structure:   &ContractByteCode{},
			Description: "合约编译文件记录表",
		},
		{
			Name:        TableGas,
			Structure:   &Gas{},
		...
		{
			Name:        TableContractVerifyResult,
			Structure:   &ContractVerifyResult{},
			Description: "合约源码验证结果表",
		},
		{
			Name:        TableContractSourceFile,
			Structure:   &ContractSourceFile{},
			Description: "合约源码存储表",
		},
		{
			Name:        TableDIDDetail,
		...
		{
			Name:        TableDIDSetHistory,
			Structure:   &DIDSetHistory{},
			Description: "did设置历史表",
		},
		{
			Name:        TableVCIssueHistory,
			Structure:   &VCIssueHistory{},
			Description: "vc发行历史表",
		},
		{
			Name:        TableVCTemplate,
			Structure:   &VCTemplate{},
			Description: "vc模板表",
		},
		{
			Name:        TableContractABIFile,
			Structure:   &ContractABIFile{},
			Description: "合约ABI文件表",
		},
		{
			Name:        TableContractABITopic,
			Structure:   &ContractABITopic{},
			Description: "合约ABI主题表",
		},
		{
			Name:        TableContractCrossCall,
			Structure:   &ContractCrossCall{},
			Description: "链跨合约调用关系统计",
		},
		{
			Name:        TableContractCrossCallTransaction,
			Structure:   &ContractCrossCallTransaction{},
			Description: "链跨合约交易统计",
		},
	}
}
...
func GetClickhouseTableOptions() map[string]string {
	uniqueOptions := map[string]string{
		...
		TableContractEventTopic:         "ENGINE=ReplacingMergeTree() ORDER BY (contractName) PRIMARY KEY (contractName)",
...
func PGSqlCreateTableWithComment(db *gorm.DB, chainId string, tableInfo TableInfo) error {
	tableName := GetTableName(chainId, tableInfo.Name)
	if GormDB.Migrator().HasTable(tableName) {
		//表已经存在了，直接跳过
		return nil
	...
	// 添加表注释
	err = db.Exec(fmt.Sprintf("COMMENT ON TABLE %s IS '%s'", tableName, tableInfo.Description)).Error
	if err != nil {
		return nil
	}
