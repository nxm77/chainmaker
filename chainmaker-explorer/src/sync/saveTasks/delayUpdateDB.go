...
// SaveEventTopicData 更新IDA合约
func SaveEventTopicData(chainId string, inserts, updates []*db.ContractEventTopic) error {
	if len(inserts) == 0 && len(updates) == 0 {
		return nil
	}

	err := dbhandle.InsertContractEventTopic(chainId, inserts)
	if err != nil {
		return err
	}

	for _, event := range updates {
		err := dbhandle.UpdateContractEventTopic(chainId, event)
		if err != nil {
			return err
		}
	}
...
// UpdateDIDDataToDB 函数用于将DID数据更新到数据库中
func UpdateDIDDataToDB(chainId string, didSaveDate *db.DIDSaveData) error {
	// 如果didSaveDate为空，则直接返回nil
	if didSaveDate == nil {
		return nil
	}

	// 将DID历史记录插入数据库
	err := dbhandle.InsertDIDHistorys(chainId, didSaveDate.InsertDIDHistory)
	if err != nil {
		return err
	}
	// 将VC历史记录插入数据库
	err = dbhandle.InsertVCHistorys(chainId, didSaveDate.InsertVCHistory)
	if err != nil {
		return err
	}

	// 遍历didSaveDate中的SaveDIDDetails，将DID详情插入或更新到数据库中
	for _, detail := range didSaveDate.SaveDIDDetails {
		didInfo, _ := dbhandle.GetDIDDetailById(chainId, detail.DID)
		if didInfo == nil {
			//新增
			err := dbhandle.InsertDIDDetail(chainId, detail)
			if err != nil {
				return err
			}
		} else {
			//更新
			err := dbhandle.UpdateDIDDetail(chainId, detail)
			if err != nil {
				return err
			}
		}
	...
	// 遍历didSaveDate中的UpdateDIDStatus，将DID状态更新到数据库中
	for did, status := range didSaveDate.UpdateDIDStatus {
		err := dbhandle.UpdateDIDStatus(chainId, did, status)
		if err != nil {
			return err
		}
	}
	// 遍历didSaveDate中的UpdateDIDIssuer，将DID发行者更新到数据库中
	for did, isIsuser := range didSaveDate.UpdateDIDIssuer {
		err := dbhandle.UpdateDIDIssuer(chainId, did, isIsuser)
		if err != nil {
			return err
		}
	}

	// 遍历didSaveDate中的SaveVCTemplates，将VC模板插入或更新到数据库中
	for _, template := range didSaveDate.SaveVCTemplates {
		templateDB, _ := dbhandle.GetVCTemplateById(chainId, template.TemplateID, template.ContractAddr)
		if templateDB == nil {
			//新增
			err := dbhandle.InsertVCTemplate(chainId, template)
			if err != nil {
				return err
			}
		} else {
			//更新
			err := dbhandle.UpdateVCTemplate(chainId, template)
			if err != nil {
				return err
			}
		}
	...
	// 遍历didSaveDate中的UpdateVCStatus，将VC状态更新到数据库中
	for vcId, status := range didSaveDate.UpdateVCStatus {
		err := dbhandle.UpdateVCIssuerStatus(chainId, vcId, status)
		if err != nil {
			return err
		}
	}
...
func SaveChainStatistics(chainId string, statistics *db.Statistics) error {
	if statistics == nil {
		return nil
	}

	//获取区块链统计信息
	dbStatistics, _ := dbhandle.GetChainStatistics(chainId)
	if dbStatistics == nil {
		...
		err := dbhandle.UpdateStatisticsDelay(chainId, statistics)
		if err != nil {
			return err
		}
	}
...
// SaveABITopicTableEvents函数用于保存ABI主题表事件
func SaveABITopicTableEvents(chainId string, topicTableEvents map[string][]map[string]interface{}) error {
	// 遍历topicTableEvents中的每一个tableName和events
	for tableName, events := range topicTableEvents {
		// 调用dbhandle.InsertDecodeEventByABI函数将events插入到tableName表中
		err := dbhandle.InsertDecodeEventByABI(chainId, tableName, events)
		// 如果插入失败，则返回错误
		if err != nil {
