...
func GetMaxHeight(chainId string) int64 {
	heightInterface, _ := globalChainMaxHeightMp.Load(chainId)
	height, ok := heightInterface.(int64)
	if !ok {
		log.Errorf("类型断言失败，无法将 heightInterface 转换为 int64: %v", heightInterface)
		return 0 // 或其他合适的错误处理逻辑
	}
	return height
}
