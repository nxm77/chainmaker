...
func DockerGetTotalSupply(chainId string, name string) (string, error) {
	totalSupply := "0"
	client := client.GetChainClient(chainId)
	if client == nil {
...
func EvmGetDecimals(evmType, chainId, contractAddr string) (int, error) {
	...
	txResponseJson, _ := json.Marshal(txResponse)
	log.Infof("[sdk] EvmGetDecimals txResponse: %v, contractAddr:%v err: %v", string(txResponseJson), contractAddr, err)
	if err != nil || txResponse == nil {
		return decimals, fmt.Errorf("decimals QueryContract failed err: %v,txResponse: %v", err, txResponse)
	}
...
func DockerGetContractType(chainId, contractName string) (string, error) {
	...
	chainClient := client.GetChainClient(chainId)
	if chainClient == nil {
		return contractType, fmt.Errorf("[Pool] don't analyze pool, chainId:%v", chainId)
	}

	txResponse, err := chainClient.QueryContract(contractName, "Standards", []*common.KeyValuePair{}, 100)
	if err != nil || txResponse == nil {
		log.Warnf("【sdk】DockerGetContractType method Standards err :%v, contractName:%v", err, contractName)
		return contractType, err
	}

	if txResponse.Code != common.TxStatusCode_SUCCESS {
		log.Warnf("【sdk】DockerGetContractType method Standards err, txResponse.Code:%v, contractName:%v",
	...
	if err != nil {
		log.Warnf("【sdk】DockerGetContractType method Standards err, json unmarsh err:%v, contractName:%v",
			err, contractName)
		return contractType, err
	}
...
func EVMGetContractSymbol(chainId, contractAddr, evmType string) (string, error) {
	...
	txResponseJson, _ := json.Marshal(txResponse)
	log.Infof("[sdk] EVMGetContractSymbol txResponse: %v, contractAddr:%v err: %v",
		string(txResponseJson), contractAddr, err)
	if err != nil || txResponse == nil {
...
// ExtractFunctionSignatures 函数用于从字节码中提取函数签名
...
func GetMemberIdAddrAndCertNew(chainId, hashType string, member *accesscontrol.Member) (*MemberAddrIdCert, error) {
	ret := &MemberAddrIdCert{}
	if member == nil {
		return ret, nil
	}
	...
	switch member.MemberType.String() {
	case accesscontrol.MemberType_CERT.String():
		x509Cert, userId, userAddr, err = handleCertMember(member.MemberInfo)
	case accesscontrol.MemberType_CERT_HASH.String():
		x509Cert, userId, userAddr, err = handleCertHashMember(chainId, member.MemberInfo)
	case accesscontrol.MemberType_ALIAS.String():
		x509Cert, userId, userAddr, err = handleAliasMember(chainId, member.MemberInfo)
	case accesscontrol.MemberType_PUBLIC_KEY.String():
		userId, userAddr, err = handlePublicKeyMember(hashType, member.MemberInfo)
	}
...
func handleCertMember(memberInfo []byte) (*x509.Certificate, string, string, error) {
	x509Cert, err := utils.ParseCertificate(memberInfo)
	if err != nil {
		return nil, "", "", fmt.Errorf("[Parse SerializedMember] parse cert failed: %s", err.Error())
	}

	pk, err := publicKeyFromCert(memberInfo)
	if err != nil {
		return nil, "", "", err
	}
	nodeId, err := GetNodeIdByPublicKey(pk)
	if err != nil {
		return nil, "", "", err
	}
	...
	return x509Cert, nodeId, userAddr, nil
}

func handleCertHashMember(chainId string, memberInfo []byte) (*x509.Certificate, string, string, error) {
	chainClient := client.GetChainClient(chainId)
	if chainClient == nil {
		err := fmt.Errorf("ParseCertificate get chainClient failed:  chainId%s", chainId)
		return nil, "", "", err
	}
	certInfos, err := chainClient.QueryCert([]string{hex.EncodeToString(memberInfo)})
	if err != nil {
		err = fmt.Errorf("QuertCert failed: %v", err)
		return nil, "", "", err
	}
...
func handleAliasMember(chainId string, memberInfo []byte) (*x509.Certificate, string, string, error) {
	chainClient := client.GetChainClient(chainId)
	if chainClient == nil {
		err := fmt.Errorf("parseCertificate get chainClient failed:  chainId%s", chainId)
		return nil, "", "", err
	...
	aliasInfos, errAli := chainClient.QueryCertsAlias([]string{string(memberInfo)})
	if errAli != nil {
		err := fmt.Errorf("quert AliasInfos failed: %v ", errAli)
		return nil, "", "", err
	}
...
func handlePublicKeyMember(hashType string, memberInfo []byte) (string, string, error) {
	//publicKeyStr := member.MemberInfo
	chainHashType := hashType
	publicKey, err := asym.PublicKeyFromPEM(memberInfo)
	if err != nil {
		return "", "", err
	}
	userAddr, err := commonutils.PkToAddrStr(publicKey, pbconfig.AddrType_ETHEREUM,
	...
	nodeId, err := GetNodeIdByPublicKey(memberInfo)
	if err != nil {
		return "", "", err
	}
	return nodeId, userAddr, nil
}

func GetNodeIdByPublicKey(publicKeyPEM []byte) (string, error) {
	// 检查输入的公钥是否为空
	if len(publicKeyPEM) == 0 {
		return "", fmt.Errorf("public key is empty")
	}

	// 将 PEM 格式的公钥字符串解析为公钥对象
	publicKeyObj, err := asym.PublicKeyFromPEM(publicKeyPEM)
	if err != nil {
		return "", fmt.Errorf("failed to parse public key: %v", err)
	}

	// 根据公钥对象生成节点 UID
	nodeUid, err := helper.CreateLibp2pPeerIdWithPublicKey(publicKeyObj)
	if err != nil {
		return "", fmt.Errorf("failed to create node UID: %v", err)
	}
...
func publicKeyFromCert(member []byte) ([]byte, error) {
	certificate, err := commonutils.ParseCert(member)
	if err != nil {
		return nil, err
	}
	pubKeyStr, err := certificate.PublicKey.String()
	//log.Infof("handleCertMember =======pubKeyStr: %v", pubKeyStr)
	if err != nil {
		return nil, err
	}
