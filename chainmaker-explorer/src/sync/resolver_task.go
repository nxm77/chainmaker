...
//	@return error
func DelayParallelParseGetDB(chainId string, delayedUpdateCache *model.GetRealtimeCacheData,
	contractMap map[string]*db.Contract, topicEventResult *model.TopicEventResult,
	crossSubChainIdMap, eventTopicTxNum map[string]map[string]int64) (*model.GetDBResult, error) {
	...
	getDBResult := &model.GetDBResult{
		...
		EventTopicMap:          make(map[string]map[string]*db.ContractEventTopic, 0),
	}

	var didUnBinds []string
	if topicEventResult.DIDEventData != nil {
		didUnBinds = topicEventResult.DIDEventData.DIDUnBinds
	}
	// 定义一个任务列表
	tasks := []TaskSync{
		...
		getAccountByDIDListTask(chainId, getDBResult, didUnBinds),
		...
		getEventTopicListTask(chainId, getDBResult, eventTopicTxNum),
	}
...
func getGasListTask(chainId string, delayedUpdateCache *model.GetRealtimeCacheData,
	getDBResult *model.GetDBResult) TaskSync {
	return TaskFunc(func() error {
		addrList := logic.BuildGasAddrList(delayedUpdateCache.GasRecords)
...
//	@return TaskSync
func getPositionMapTask(chainId string, getDBResult *model.GetDBResult, ownerAdders []string) TaskSync {
	return TaskFunc(func() error {
		//根据owner获取合约信息
...
//	@param ownerAdders 账户地址
//	@return TaskSync
func getNonPositionMapTask(chainId string, getDBResult *model.GetDBResult, ownerAdders []string) TaskSync {
	return TaskFunc(func() error {
		//根据owner获取合约信息
...
//	@return TaskSync
func getFungibleContractTask(chainId string, getDBResult *model.GetDBResult,
...
//	@param contractMap 本次涉及全部合约
//	@return TaskSync
func getNonFungibleContractTask(chainId string, getDBResult *model.GetDBResult,
	contractMap map[string]*db.Contract) TaskSync {
	return TaskFunc(func() error {
		nonFungibleAddr := make([]string, 0)
		for _, contract := range contractMap {
			if contract.ContractType == common.ContractStandardNameCMNFA ||
...
//	@return TaskSync
func getAddBlackTxListTask(chainId string, getDBResult *model.GetDBResult, addBlackTxIds []string) TaskSync {
	return TaskFunc(func() error {
		//添加黑名单交易
...
//	@return TaskSync
func getDeleteBlackTxListTask(chainId string, getDBResult *model.GetDBResult, deleteBlackTxIds []string) TaskSync {
	return TaskFunc(func() error {
		//删除黑名单交易
...
//	@return TaskSync
func getSubChainCrossListTask(chainId string, getDBResult *model.GetDBResult,
...
//	@param crossSubChainIdMap 子链列表
//	@return TaskSync
func getSubChainDBMapTask(chainId string, getDBResult *model.GetDBResult,
	crossSubChainIdMap map[string]map[string]int64) TaskSync {
	return TaskFunc(func() error {
		var subChainIds []string
		for subChainId := range crossSubChainIdMap {
...
//	@param crossSubChainIdMap 子链列表
//	@return TaskSync
func getAccountByBNSListTask(chainId string, getDBResult *model.GetDBResult, bnsUnBindDomain []string) TaskSync {
	return TaskFunc(func() error {
...
//	@param crossSubChainIdMap 子链列表
//	@return TaskSync
func getAccountByDIDListTask(chainId string, getDBResult *model.GetDBResult, didUnBindList []string) TaskSync {
	return TaskFunc(func() error {
...
//	@return TaskSync
func getAccountMapTask(chainId string, getDBResult *model.GetDBResult, topicEventResult *model.TopicEventResult,
	userInfoMap map[string]*db.User) TaskSync {
	return TaskFunc(func() error {
		if topicEventResult == nil {
			return nil
		}
		//BNS账户
		...
		if topicEventResult.DIDEventData != nil {
			//did账户
			for _, account := range topicEventResult.DIDEventData.Account {
				topicEventResult.OwnerAdders = append(topicEventResult.OwnerAdders, account.Address)
			}
			//获取解绑用户
			unBindDIDList, errDID := dbhandle.GetAccountByDIDList(chainId, topicEventResult.DIDEventData.DIDUnBinds)
			if errDID != nil {
				return errDID
			}
		...
		//获取解绑用户
		unBindBNSList, errBns := dbhandle.GetAccountByBNSList(chainId, topicEventResult.BNSUnBindDomain)
		if errBns != nil {
			return errBns
		}
...
//	@param contractMap 本次涉及全部合约
//	@return TaskSync
func getIDAContractTask(chainId string, getDBResult *model.GetDBResult, idaEventData *model.IDAEventData) TaskSync {
	return TaskFunc(func() error {
		if idaEventData == nil {
			return nil
		}
...
func getIDAAssetDetailsTask(chainId string, getDBResult *model.GetDBResult,
	idaEventData *model.IDAEventData) TaskSync {
	return TaskFunc(func() error {
		if idaEventData == nil {
			return nil
		}
...
func getEventTopicListTask(chainId string, getDBResult *model.GetDBResult,
	eventTopicTxNum map[string]map[string]int64) TaskSync {
	return TaskFunc(func() error {
		eventTopicTxNumDB := make(map[string]map[string]*db.ContractEventTopic, 0)
		if len(eventTopicTxNum) == 0 {
			return nil
		}

		contractNames := make([]string, 0)
		for contractName := range eventTopicTxNum {
			contractNames = append(contractNames, contractName)
		}

		//非同质化合约
		eventTopicList, err := dbhandle.GetEventTopicByNames(chainId, contractNames)
		if err != nil {
			return err
		}

		for _, event := range eventTopicList {
			// 初始化内层映射
			if eventTopicTxNumDB[event.ContractName] == nil {
				eventTopicTxNumDB[event.ContractName] = make(map[string]*db.ContractEventTopic)
			}
			// 统计 Topic 的出现次数
			eventTopicTxNumDB[event.ContractName][event.Topic] = event
		}

		getDBResult.EventTopicMap = eventTopicTxNumDB
		return nil
	})
}
