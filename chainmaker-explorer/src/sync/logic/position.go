...
func BuildPositionList(contractEvents []*db.ContractEventData, contractInfoMap map[string]*db.Contract,
	...
	for _, event := range contractEvents {
		...
		if contract.ContractType != common.ContractStandardNameCMDFA &&
			...
			contract.ContractType != common.ContractStandardNameCMNFA &&
		...
		if contract.ContractType == common.ContractStandardNameCMDFA ||
			contract.ContractType == common.ContractStandardNameEVMDFA {
			amountDecimal = common.StringAmountDecimal(amount, contract.Decimals)
		}
...
func BuildUpdatePositionData(minHeight int64, positionList map[string]*db.PositionData,
	...
	for _, position := range positionList {
		if position.ContractType == common.ContractStandardNameCMDFA ||
			position.ContractType == common.ContractStandardNameEVMDFA {
			//同质化
			dealFungiblePosition(minHeight, position, positionDBMap, positionOp)
		} else if position.ContractType == common.ContractStandardNameCMNFA ||
			position.ContractType == common.ContractStandardNameEVMNFA {
			//非同质化
