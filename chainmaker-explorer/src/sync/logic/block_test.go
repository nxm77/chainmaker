...
func TestBuildLatestBlockListCache1(t *testing.T) {
	type args struct {
		chainId  string
		modBlock *db.Block
	}
...
func TestDealBlockInfo(t *testing.T) {
	// Test case 1: Normal case with valid block info
	blockInfo1 := &pbCommon.BlockInfo{
		Block: &pbCommon.Block{
			Header: &pbCommon.BlockHeader{
				...
				BlockVersion:   1,
				PreBlockHash:   []byte("preBlockHash1"),
				ConsensusArgs:  []byte("consensusArgs1"),
				DagHash:        []byte("dagHash1"),
				BlockTimestamp: 1625097600,
				TxCount:        1,
				RwSetRoot:      []byte("rwSetRoot1"),
	...
	err1 := DealBlockInfo(blockInfo1, hashType1, dealResult1)
	if err1 != nil {
		t.Errorf("Test case 1 failed: %s", err1.Error())
	}

	// Test case 3: Proposer info is nil
	...
	err3 := DealBlockInfo(blockInfo3, hashType3, dealResult3)
	if err3 != nil {
		t.Errorf("Test case 3 failed: %s", err3.Error())
	}
}
...
func TestBuildLatestBlockListCache(t *testing.T) {
	modBlock := &db.Block{
		ID:            "block1",
		BlockHeight:   1,
		BlockHash:     hex.EncodeToString([]byte("blockHash1")),
		BlockVersion:  1,
		PreBlockHash:  hex.EncodeToString([]byte("preBlockHash1")),
		ConsensusArgs: utils.Base64Encode([]byte("consensusArgs1")),
		DagHash:       hex.EncodeToString([]byte("dagHash1")),
		Timestamp:     1625097600,
		TxCount:       1,
		RwSetHash:     hex.EncodeToString([]byte("rwSetRoot1")),
		Signature:     utils.Base64Encode([]byte("signature1")),
		TxRootHash:    hex.EncodeToString([]byte("txRoot1")),
		OrgId:         "org1",
		ProposerAddr:  "proposerAddr1",
		ProposerId:    "proposerId1",
	}

	BuildLatestBlockListCache(ChainId, modBlock)
}

func TestBuildOverviewMaxBlockHeightCache(t *testing.T) {
	blockInfo := &db.Block{
		ID:            "block1",
		BlockHeight:   1,
		BlockHash:     hex.EncodeToString([]byte("blockHash1")),
	...
	BuildOverviewMaxBlockHeightCache(ChainId, blockInfo)
}

func TestDealBlockchainStatistics(t *testing.T) {
	// Test case 1: Normal case with valid parameters
	insertAccount1 := []*db.Account{}
	_ = DealBlockchainStatistics(db.UTchainID, insertAccount1)

	// Test case 2: Nil chainId
	insertAccount2 := []*db.Account{}
	_ = DealBlockchainStatistics("", insertAccount2)
}

// TestDealStatisticsRealtime 测试 DealStatisticsRealtime 函数
func TestDealStatisticsRealtime(t *testing.T) {
	// 初始化测试数据
	blockHeight := int64(100)
	// 调用被测试函数
	err := DealStatisticsRealtime(db.UTchainID, blockHeight, nil)
	if err != nil {
	...
	// 调用被测试函数
	err = DealStatisticsRealtime(db.UTchainID, blockHeight, dealResult)
	// 断言没有错误
