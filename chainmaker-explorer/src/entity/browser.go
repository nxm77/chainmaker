...
type BlockTxDependenciesView struct {
	TxIds []string
	Dag   []*pbCommon.DAG_Neighbor
}
...
// Version Version
type ContractTypesView struct {
	ContractTypes []string
}

// OpenLicenseTypeView OpenLicenseTypeView
type OpenLicenseTypeView struct {
	LicenseTypes []string
}

// VerifyContractView
type VerifyContractView struct {
	VerifyStatus int    //0：未验证，1:验成功。2:验证失败
	MetaData     string //验证结果
	Message      string //验证失败原因
}

// ChartDqueryAPIView chart
// param view
type ChartDqueryAPIView struct {
	TaskId       string
	ChartType    string
	BusinessType string
}

// DIDDetailView detail
// param view
type DIDDetailView struct {
	DID            string
	ContractName   string
	ContractAddr   string
	Status         int
...
// PluginLoginView 插件登录
type PluginLoginView struct {
	Token    string `json:"Token"`
	UserAddr string `json:"UserAddr"`
}

type CheckLoginView struct {
	UserAddr string `json:"UserAddr"`
}

type ABIDetailView struct {
	ContractName    string
	ContractAddr    string
	ContractVersion string
	ABIJson         string
...
type ContractCrossCallView struct {
	CurrentContract      string
	InvokingContracts    []string
	TargetContracts      []string
	MethodTargetContract []MethodTargetContract
}

type MethodTargetContract struct {
	Method          string
	TargetContracts []string
}
